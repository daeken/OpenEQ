...
namespace OpenEQ.Network {
    public static class Utility {
        ...
        static uint UpdateCRC(byte[] data, uint crc = 0xFFFFFFFF) {
            foreach(var x in data)
                crc = (crc >> 8) ^ crcTable[x ^ (crc & 0xFF)];
            return crc;
        }
        public static ushort CalculateCRC(byte[] data, byte[] key) {
            return (ushort)~UpdateCRC(data, UpdateCRC(key));
        }
    }
}
