...
namespace Jitter {
    ...
    public class World {
		...
		readonly Queue<Arbiter> addedArbiterQueue = new Queue<Arbiter>();

		float angularDamping = 0.85f;

		readonly Action<object> arbiterCallback;

		readonly CollisionDetectedHandler collisionDetectionHandler;
		readonly HashSet<Constraint> constraints = new HashSet<Constraint>();

		int contactIterations = 10;

		float currentAngularDampFactor = 1.0f;

		float currentLinearDampFactor = 1.0f;
		float deactivationTime = 2f;

		float inactiveAngularThresholdSq = 0.1f;
		float inactiveLinearThresholdSq = 0.1f;
		readonly Action<object> integrateCallback;

		readonly IslandManager islands = new IslandManager();
		float linearDamping = 0.85f;

		readonly Queue<Arbiter> removedArbiterQueue = new Queue<Arbiter>();

		readonly Stack<Arbiter> removedArbiterStack = new Stack<Arbiter>();

		readonly HashSet<RigidBody> rigidBodies = new HashSet<RigidBody>();
		int smallIterations = 4;
		readonly HashSet<SoftBody> softbodies = new HashSet<SoftBody>();

		readonly ThreadManager threadManager = ThreadManager.Instance;
		float timestep;
        ...
        ///     The collisionSystem which is used to detect
        ...
        public void Clear() {
			...
			foreach(var body in softbodies) CollisionSystem.RemoveEntity(body);

			// remove bodies from the world
			rigidBodies.Clear();

			// remove constraints
			foreach(var constraint in constraints) Events.RaiseRemovedConstraint(constraint);
			constraints.Clear();
        ...
        /// <param name="smallIterations">
        ///     The number of contact iteration used for smaller (two and three constraint) systems.
        ///     Default value 4.
        /// </param>
        /// <remarks>
        ///     The number of iterations for collision and contact should be between 3 - 30.
        ///     More iterations means more stability and also a longer calculation time.
        /// </remarks>
        ...
        public bool RemoveBody(RigidBody body) => RemoveBody(body, false);

		bool RemoveBody(RigidBody body, bool removeMassPoints) {
        ...
        public void Step(float timestep, bool multithread) {
			...
			// Calculate this
			currentAngularDampFactor = MathF.Pow(angularDamping, timestep);
			currentLinearDampFactor = MathF.Pow(linearDamping, timestep);
		...
		void UpdateArbiterContacts(Arbiter arbiter) {
		...
		void UpdateContacts() {
			foreach(var arbiter in ArbiterMap.Arbiters) UpdateArbiterContacts(arbiter);
		...
		void ArbiterCallback(object obj) {
		...
		void HandleArbiter(int iterations, bool multiThreaded) {
			if(multiThreaded) {
		...
		void IntegrateForces() {
			foreach(var body in rigidBodies) {
				if(!body.isStatic && body.IsActive) {
					Vector3 temp;
					temp = body.force * (body.inverseMass * timestep);
					body.linearVelocity = temp + body.linearVelocity;

					if(!body.isParticle) {
						temp = body.torque * timestep;
						temp = temp.Transform(ref body.invInertiaWorld);
		...
		void IntegrateCallback(object obj) {
			var body = obj as RigidBody;

			Vector3 temp;
			temp = body.linearVelocity * timestep;
			...
			if(!body.isParticle) {
				...
				if(angle < 0.001f)
					...
					axis = body.angularVelocity * (MathF.Sin(0.5f * angle * timestep) / angle);

				var dorn = new JQuaternion(axis.X, axis.Y, axis.Z, MathF.Cos(angle * timestep * 0.5f));
				JQuaternion ornA;
		...
		void Integrate(bool multithread) {
			if(multithread) {
				foreach(var body in rigidBodies) {
		...
		void CollisionDetected(RigidBody body1, RigidBody body2, Vector3 point1, Vector3 point2, Vector3 normal,
			float penetration) {
        ...
        /// </summary>
        /// <example>
        ///     int time = DebugTimes[(int)DebugType.CollisionDetect] gives
        ///     the amount of time spent on collision detection during the last <see cref="Step" />.
        /// </example>
        readonly double[] debugTimes = new double[(int) DebugType.Num];
