...
namespace Jitter.Collision {
    ...
    public class Octree {
		...
		ArrayResourcePool<ushort> nodeStackPool;
		...
		public void BuildOctree() {
			...
			for(var i = 0; i < tris.Length; i++) {
				Extensions.Min(ref positions[tris[i].I1], ref positions[tris[i].I2], out triBoxes[i].Min);
				Extensions.Min(ref positions[tris[i].I0], ref triBoxes[i].Min, out triBoxes[i].Min);

				Extensions.Max(ref positions[tris[i].I1], ref positions[tris[i].I2], out triBoxes[i].Max);
				Extensions.Max(ref positions[tris[i].I0], ref triBoxes[i].Max, out triBoxes[i].Max);

				// get size of the root box
				Extensions.Min(ref rootNodeBox.Min, ref triBoxes[i].Min, out rootNodeBox.Min);
				Extensions.Max(ref rootNodeBox.Max, ref triBoxes[i].Max, out rootNodeBox.Max);
			}
		...
		void CreateAABox(ref JBBox aabb, EChild child, out JBBox result) {
			var dims = (aabb.Max - aabb.Min) / 2;
			...
			result = new JBBox();
			result.Min = new Vector3(offset.X * dims.X, offset.Y * dims.Y, offset.Z * dims.Z) + aabb.Min;
		...
		void GatherTriangles(int nodeIndex, ref List<int> tris) {
			...
			while(curStackIndex < endStackIndex) {
				...
				if(nodes[nodeIndex].box.Contains(ref testBox) != JBBox.ContainmentType.Disjoint) {
					for(var i = 0; i < nodes[nodeIndex].triIndices.Length; ++i)
						if(triBoxes[nodes[nodeIndex].triIndices[i]].Contains(ref testBox) !=
						   JBBox.ContainmentType.Disjoint) {
