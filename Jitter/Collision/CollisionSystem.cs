...
namespace Jitter.Collision {
    ...
    /// <returns>
    ///     If false is returned the collision information is dropped. The CollisionDetectedHandler
    ///     is never called.
    /// </returns>
    ...
    ///     ray the collision occured. The hitPoint is calculated by: rayStart+friction*direction.
    /// </param>
    ...
    public abstract class CollisionSystem {
        ...
        /// </summary>
        public abstract bool Raycast(Vector3 rayOrigin, Vector3 rayDirection, RaycastCallback raycast,
        ...
        ///     which start at rayOrigin and end in rayOrigin + rayDirection.
        /// </summary>
        public abstract bool Raycast(RigidBody body, Vector3 rayOrigin, Vector3 rayDirection, out Vector3 normal,
			out float fraction);
        ...
        public bool CheckBothStaticOrInactive(IBroadphaseEntity entity1, IBroadphaseEntity entity2) =>
			entity1.IsStaticOrInactive && entity2.IsStaticOrInactive;
        ...
        public bool CheckBoundingBoxes(IBroadphaseEntity entity1, IBroadphaseEntity entity2) {
			...
			return box1.Max.Z >= box2.Min.Z && box1.Min.Z <= box2.Max.Z && box1.Max.Y >= box2.Min.Y &&
			       box1.Min.Y <= box2.Max.Y && box1.Max.X >= box2.Min.X && box1.Min.X <= box2.Max.X;
		}
		...
		readonly ResourcePool<List<int>> potentialTriangleLists = new ResourcePool<List<int>>();

		void DetectSoftSoft(SoftBody body1, SoftBody body2) {
			...
			for(var i = 0; i < other.Count; i++) {
				...
				result = XenoCollide.Detect(myTriangle, otherTriangle, ref JMatrix.InternalIdentity,
		...
		void DetectRigidRigid(RigidBody body1, RigidBody body2) {
			var b1IsMulti = body1.Shape is Multishape;
			var b2IsMulti = body2.Shape is Multishape;

			var speculative = EnableSpeculativeContacts || body1.EnableSpeculativeContacts ||
			                  body2.EnableSpeculativeContacts;
			...
			if(!b1IsMulti && !b2IsMulti) {
				if(XenoCollide.Detect(body1.Shape, body2.Shape, ref body1.orientation,
					ref body2.orientation, body1.position, body2.position,
					out point, out normal, out penetration)) {
					Vector3 point1, point2;
					FindSupportPoints(body1, body2, body1.Shape, body2.Shape, ref point, ref normal, out point1,
						out point2);
						...
						if(XenoCollide.Detect(ms1, ms2, ref body1.orientation,
							...
							if(GJKCollide.ClosestPoints(ms1, ms2, ref body1.orientation, ref body2.orientation,
								...
								if(delta.LengthSquared() <
								   (body1.sweptDirection - body2.sweptDirection).LengthSquared()) {
									penetration = Vector3.Dot(delta, normal);

									if(penetration < 0.0f)
										RaiseCollisionDetected(body1, body2, ref hit1, ref hit2, ref normal,
											penetration);
								}
							}
						}
				...
				for(var i = 0; i < msLength; i++) {
					...
					if(XenoCollide.Detect(ms, b2.Shape, ref b1.orientation,
						ref b2.orientation, b1.position, b2.position,
						out point, out normal, out penetration)) {
						Vector3 point1, point2;
						...
						if(useTerrainNormal && ms is TerrainShape) {
							...
							normal = normal.Transform(ref b1.orientation);
		...
		void DetectSoftRigid(RigidBody rigidBody, SoftBody softBody) {
			if(rigidBody.Shape is Multishape) {
				...
				foreach(var i in detected) {
					...
					result = XenoCollide.Detect(rigidBody.Shape, t, ref rigidBody.orientation,
						ref JMatrix.InternalIdentity,
						rigidBody.position, Vector3.Zero, out point, out normal, out penetration);
		...
		void FindSupportPoints(RigidBody body1, RigidBody body2,
			...
			point1 = point + normal * dot1;
			point2 = point + normal * dot2;
		}

		void SupportMapping(RigidBody body, Shape workingShape, ref Vector3 direction, out Vector3 result) {
			direction.Transform(ref body.invOrientation, out result);
			workingShape.SupportMapping(ref result, out result);
			result.Transform(ref body.orientation, out result);
