...
namespace Jitter.Collision {
    ...
    public class CollisionSystemPersistentSAP : CollisionSystem {
		const int AddedObjectsBruteForceIsUsed = 250;

		int addCounter;

		readonly List<SweepPoint> axis1 = new List<SweepPoint>();
		readonly List<SweepPoint> axis2 = new List<SweepPoint>();
		readonly List<SweepPoint> axis3 = new List<SweepPoint>();
		...
		readonly HashSet<OverlapPair> fullOverlaps = new HashSet<OverlapPair>();
		...
		void SortAxis(List<SweepPoint> axis) {
			...
			public float Value {
				get {
					if(Begin) {
						...
						if(Axis == 1) return Body.BoundingBox.Min.Y;
						return Body.BoundingBox.Min.Z;
					}

					if(Axis == 0) return Body.BoundingBox.Max.X;
					if(Axis == 1) return Body.BoundingBox.Max.Y;
		...
		struct OverlapPair {
		    ...
		    /// </summary>
		    /// <returns></returns>
		    public override int GetHashCode() => Entity1.GetHashCode() + Entity2.GetHashCode();
		}
		...
		int QuickSort(SweepPoint sweepPoint1, SweepPoint sweepPoint2) {
		...
		void DirtySortAxis(List<SweepPoint> axis) {
			axis.Sort(QuickSort);
			activeList.Clear();
		...
		//        float dist1 = MathF.Abs(distance1);
		//        float dist2 = MathF.Abs(distance2);
		//        float dist3 = MathF.Abs(distance3);
