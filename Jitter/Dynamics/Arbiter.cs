...
namespace Jitter.Dynamics {
    ...
    public class Arbiter {
        ...
        /// </summary>
        public Arbiter() => contactList = new ContactList();
        ...
        public Contact AddContact(Vector3 point1, Vector3 point2, Vector3 normal, float penetration,
			...
			lock(contactList) {
				...
				contact.Initialize(body1, body2, ref point1, ref point2, ref normal, penetration, true,
					contactSettings);
		...
		void ReplaceContact(ref Vector3 point1, ref Vector3 point2, ref Vector3 n, float p, int index,
			ContactSettings contactSettings) {
		...
		int GetCacheEntry(ref Vector3 realRelPos1, float contactBreakThreshold) {
			var shortestDist = contactBreakThreshold * contactBreakThreshold;
			var size = contactList.Count;
			var nearestPoint = -1;
			for(var i = 0; i < size; i++) {
				Vector3 diffA;
				diffA = contactList[i].relativePos1 - realRelPos1;
		...
		int SortCachedPoints(ref Vector3 realRelPos1, float pen) {
			...
			if(maxPenetrationIndex != 0) {
				Vector3 a0;
				a0 = realRelPos1 - contactList[1].relativePos1;
				Vector3 b0;
				b0 = contactList[3].relativePos1 - contactList[2].relativePos1;
				Vector3 cross;
				cross = Vector3.Cross(a0, b0);
				res0 = cross.LengthSquared();
			}

			if(maxPenetrationIndex != 1) {
				Vector3 a0;
				a0 = realRelPos1 - contactList[0].relativePos1;
				Vector3 b0;
				b0 = contactList[3].relativePos1 - contactList[2].relativePos1;
				Vector3 cross;
				cross = Vector3.Cross(a0, b0);
			...
			if(maxPenetrationIndex != 2) {
				...
				a0 = realRelPos1 - contactList[0].relativePos1;
				Vector3 b0;
				b0 = contactList[3].relativePos1 - contactList[1].relativePos1;
				Vector3 cross;
				cross = Vector3.Cross(a0, b0);
			...
			if(maxPenetrationIndex != 3) {
				...
				a0 = realRelPos1 - contactList[0].relativePos1;
				Vector3 b0;
				b0 = contactList[2].relativePos1 - contactList[1].relativePos1;
				Vector3 cross;
				cross = Vector3.Cross(a0, b0);
