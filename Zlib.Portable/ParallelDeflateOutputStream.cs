...
namespace Ionic.Zlib {
    ...
    public class ParallelDeflateOutputStream : Stream {
		...
		void _InitializePoolOfWorkItems() {
			...
			var nTasks = BufferPairsPerCore * SomeFantansyNumberOfAvailableCpuCores;
					...
					if(Monitor.TryEnter(_toWrite, millisecondsToWait)) {
						...
						if(nextToWrite >= 0) {
							...
							if(workitem.ordinal != _lastWritten + 1) {
								...
								lock(_toWrite) _toWrite.Enqueue(nextToWrite);
		...
		void _DeflateOne(object wi) {
			...
			try {
				...
				lock(_toWrite) _toWrite.Enqueue(workitem.index);
		...
		void TraceOutput(TraceBits bits, string format, params object[] varParams) {
			if((bits & _DesiredTrace) != 0)
				lock(_outputLock) {
					...
					Debug.WriteLine("{0:000} PDOS ", tid);
